...
class Subsession(BaseSubsession):
    def creating_session(self):
        ...
        if self.round_number == 1:
            self.session.vars["punishment_condition"] = random.random() < 0.5
            print(
                "Punishment condition!"
                if self.session.vars["punishment_condition"]
                else "No punishment condition!"
            )
...
class Group(BaseGroup):
    ...
    def set_first_stage_earnings(self):
        ...
        for p in players:
            p.payoff_from_private = p.private_investment

            p.payoff_from_public = (
                Constants.efficiency_factor * self.total_group_investment
            )
...
class Player(BasePlayer):
    ...
    private_investment = models.CurrencyField(
        min=0, verbose_name="How much would you like to invest in private account?"
    )

    punishment_sent_to_player_1 = models.IntegerField(
        min=0,
        max=10,
        initial=None,
        label="Choose punishment for player #1",
    )
    punishment_sent_to_player_2 = models.IntegerField(
        min=0, max=10, initial=None, label="Choose punishment for player #2"
    )
    punishment_sent_to_player_3 = models.IntegerField(
        min=0, max=10, initial=None, label="Choose punishment for player #3"
    )
    punishment_sent_to_player_4 = models.IntegerField(
        min=0, max=10, initial=None, label="Choose punishment for player #4"
    )
    punishment_sent_to_player_5 = models.IntegerField(
        min=0, max=10, initial=None, label="Choose punishment for player #5"
    )
    ...
    def set_punishment_and_final_payoffs(self):
        ...
        for other_player in self.get_others_in_group():
            ...
            punishment_points = getattr(
                self, f"punishment_sent_to_player_{other_player.id_in_group}"
            )
