...
class PunishmentPage(Page):
    ...
    def is_displayed(self):
        return self.session.group.punishment_condition or punishment_condition in self.session.config
...
class FinalGameResults(Page):
    ...
    def vars_for_template(self):
        ...
        player_accumulated_payoff = sum(
            (player.payoff if self.session.vars["punishment_condition"] else player.gross_profit)
