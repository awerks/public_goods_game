...
class Contribution(Page):
    form_model = "player"
    form_fields = ["public_investment", "private_investment"]
...
class ObservationPage(Page):
    def vars_for_template(self):
        players_in_all_rounds = [
            player.in_all_rounds() for player in self.group.get_players()
        ]
...
class PunishmentPage(Page):
    ...
    def get_form_fields(self):
        self.other_players = [
            f"punishment_sent_to_player_{i.id_in_group}"
    ...
    def is_displayed(self):
        return self.session.vars["punishment_condition"]
...
class FinalRoundResults(Page):
    ...
    def vars_for_template(self):
        accumulated_payoff = sum(
            player.payoff for player in self.player.in_all_rounds()
        )
        return dict(
            punishment_reduction_percentage=min(
                1, int(self.player.received_punishment) / 10
            )
            * 100,
            round_number=self.round_number,
...
class FinalGameResults(Page):
    ...
    def vars_for_template(self):
        # group_accumulated_payoff = [
        #     sum(p.payoff for p in player.in_all_rounds())
        #     for player in self.group.get_players()
        # ]
