...
class Constants(BaseConstants):
    ...
    players_per_group = 3
    num_rounds = 7
    num_recent_rounds_to_display = 5
    endowment = 20
    ...
    def creating_session(self):
        ...
        if self.round_number == 1:
            ...
            for i, group in enumerate(self.get_groups(), start=1):
                ...
                print(f"Group {i} (group-id: {group.id}) has{'' if choice else ' no'} punishment condition")
                group.punishment_condition = choice


class Group(BaseGroup):
    total_group_investment = models.CurrencyField(initial=0)
    punishment_condition = models.BooleanField(initial=False)
