...
class IntroductionPage(Page):
    def vars_for_template(self):
        return dict(
            punishment_condition=self.group.punishment_condition or "punishment_condition" in self.session.config,
        )
...
class ObservationPage(Page):
    def vars_for_template(self):
        ...
        # Define how many recent rounds to include.
        num_recent_rounds = 5  # For example, the last 5 rounds.

        # Compute the first round to display (cannot be less than 1)
        first_round_to_display = max(1, current_round - Constants.num_recent_rounds + 1)
...
class PunishmentPage(Page):
    ...
    def is_displayed(self):
        return self.group.punishment_condition or "punishment_condition" in self.session.config
...
class FinalGameResults(Page):
    ...
    def vars_for_template(self):
        ...
        player_accumulated_payoff = sum(
            (
                player.payoff
                if self.group.punishment_condition or "punishment_condition" in self.session.config
